...
interface DeleteBudgetItemModalProps {
...
export function DeleteBudgetItemModal({ item, isOpen, onClose, onConfirm }: DeleteBudgetItemModalProps) {
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      setErrorMessage(err.message || 'Error al eliminar la partida presupuestaria.');
    } finally {
      setIsDeleting(false);
    }
  };
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-on-surface/30 backdrop-blur-xs animate-in fade-in duration-200">
      ...
      <div className="relative w-full max-w-md bg-surface-container-lowest border border-outline-variant/60 rounded-2xl shadow-2xl overflow-hidden z-10 p-6 animate-in zoom-in-95 duration-200">
        <div className="flex items-center gap-3 mb-4 text-rose-600 dark:text-rose-400">
          ...
          <div>
            <h3 className="font-headline-sm text-lg font-bold text-on-surface">Eliminar Partida Presupuestaria</h3>
        ...
        <p className="text-sm text-secondary mb-6 leading-relaxed">
          ¿Estás seguro de que deseas eliminar la partida <strong className="text-on-surface">{item.category}</strong> de tu presupuesto?
        </p>
